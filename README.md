...
Untrusted input is a message; it can only ever become one of a fixed set of decisions over a
...
- **Untrusted input:** `fetch<web>` — a message
- **Closed type:** `type Decision = React(Emoji) | Ignore` where `Emoji` is closed
- **Sink / capability:** `grant react` → `privileged { react(d) }`
- **Consequence axes:** Trust

## Run the demo

```sh
examples/apps/reaction-bot/demo.sh
```
...
- `reaction-bot_safe.eide` — the correct design.
- `reaction-bot_unsafe.eide` — the tempting-but-wrong version (the negative example a model must learn to reject).
